...
- **Infrastructure (`Simrun.Infrastructure`)** — adapters implementing ports (in-memory levels, naive physics, system time, run store). Swap these without touching domain code.
- **Presentation (`Simrun.Presentation`)** — the host/composition root. Today it's a console loop; later it can be a Unity/Godot/Stride front-end that still talks only to the application layer.
...
- Replace `InMemoryLevelRepository` with authored level assets or procedural generators.
